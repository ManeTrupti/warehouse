...
class MDMService:
    ...
    BASE_URL = settings.MDM_SERVICE_URL
    ...
    def _summarize_error_response(cls, response_data):
        ...
        if "<html" in response_data.lower():
            ...
            if match:
                ...
                summary = "HTML error page returned from MDM service"
        ...
        try:
            ...
            if response.status_code >= 500:
                raise APIException(detail={
                    "message": "MDM service not working",
            ...
            logger.exception(
                "MDM service request failed. url=%s method=%s params=%s payload=%s status_code=%s body=%s",
                url,
                method,
                params,
                payload,
            ...
            raise APIException(detail={
                "message": "MDM service not working",
                "error": str(e),
                "mdm_status_code": upstream_status_code,
                "mdm_response_body": upstream_body
    ...
    @classmethod
    def get_store(cls, store_code):
        """
        Fetch store details from MDM
        """
        ...
        if not data:
            raise Exception(f"Store '{store_code}' not found in MDM")
